...
export const RegistraPickingsService = (
  entrega,
  posEnterga,
  material,
  lote,
...
export const mt = {
    ...
    try {
      ...
      const config = {
        ...
        headers: {
          ...
          Cookie:
            "SAP_SESSIONID_CSQ_310=TIU_-oIogSbv4aXpVA4lEBkxk6_4NBHvvY3ncTKwTus%3d; sap-usercontext=sap-client=310",
        },
      };
      ...
      // Manejo de errores
      console.error(
        "Error al obtener la información de ingreso:",
        error.message
      );
      return { error: error.message };
    }
  },
  async registrarIngreso(data, token) {
      ...
      // Configuración de la solicitud
         ...
         let config = {
           ...
           headers: {
             ...
             'Content-Type': 'application/json',
             Authorization: 'Basic Ym9ydGVnYTpCb3J0ZWdhKzExMw==',
             Cookie: process.env.COOKIE_ODATA
           },
           data : JSON.stringify(data)
         };
         
         const response = await axios.request(config);
         const { __metadata, BodyEtiqSet, ...datosLimpios } =
         response.data.d;
       return datosLimpios; // Retornar los datos limpios
         return(response.data);
    } catch (error) {
  ...
  async GetToken(data) {
    let config = {
      method: "get",
      maxBodyLength: Infinity,
      url: `${process.env.SAP_DOMAIN_ODATA}ZWM_ALISTAMIENTO_SRV/`,
      headers: {
        "X-CSRF-Token": "fetch",
        "Content-Type": "application/json",
        Authorization: "Basic Ym9ydGVnYTpCb3J0ZWdhKzExMw==",
