...
export const RegistraVehiculo = async (
  tipo,
  Placa,
  Marca,
  Modelo,
  ...
  const listaSalas = sql
    .connect(configVselect)
    .then((pool) => {
      return pool
        .request()
...
export const Consultar_documentoEntrega_model = async (entrega) => {
  ...
  const docEntrega = sql
    .connect(configMSSQLServ_appdespacho)
    .then((pool) => {
      return pool
        .request()
...
export const Lista_TiposNovedadDespacho_model = async () => {
  const listTiposNov = sql
    .connect(configMSSQLServ_appdespacho)
    .then((pool) => {
      return pool.request().execute("lista_TiposNovedadDespacho");
    })
    .then((result) => {
      //console.log('en result', result)
      let response = result.recordset;
...
export const basculaGuardarPeso = async ( nombreMaquina, peso, contador) => {  
  //console.log( nombreMaquina, peso, contador)
    const responsePeso = sql
    .connect(configLilix)
    .then((pool) => {
      return pool
        .request()
        .input("Maquina", sql.Char, nombreMaquina)
        .input("peso", sql.Float, peso)
        .input("contador", sql.Int, contador)
        .execute("grabapesobascula2");
    })
    .then((result) => {
      let response = result.recordset;
      sql.close();
